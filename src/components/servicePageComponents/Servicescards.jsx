      ...
      {serviceCategories.map((category, index) => {
        ...
        return (
          <div key={index} id={categoryId} className="mb-5 w-100">
            {/* Section Heading */}
            <h3
              className=" mb-4 fw-bold fst-italic scrollanimation animated fadeInDown adr-9"
            ...
            <div className="d-flex flex-column align-items-center gap-3 w-100">
              {category.items
                ...
                .map((row, rIndex) => (
                  <div
                    key={rIndex}
                    className="d-flex flex-lg-row flex-column gap-3 justify-content-center w-100"
                  >
                    {row.map((item, i) => (
                      <Link
                        key={i}
                        to={item.link}
                        className="card-service flex-fill p-3"
